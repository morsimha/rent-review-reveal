...
function buildYad2SearchUrl(scanParams) {
  const baseUrl = scanParams.propertyType === 'rent' ? 'https://www.yad2.co.il/realestate/rent' : 'https://www.yad2.co.il/realestate/forsale';
  ...
  if (scanParams.areas && scanParams.areas.length > 0) {
    const cityIds = scanParams.areas.map((area)=>cityMapping[area]).filter(Boolean);
    if (cityIds.length > 0) {
...
function parseYad2Html(html, scanParams) {
  const apartments = [];
  try {
